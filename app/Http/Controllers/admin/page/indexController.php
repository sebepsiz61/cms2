...
LanguageContent::InsertorUpdate($all['slug'], PAGE_LANGUAGE, SLUG_LANGUAGE, $id, 0);
   ...
   LanguageContent::InsertorUpdate($all['name'], PAGE_LANGUAGE, NAME_LANGUAGE, $id, 0);
      ...
      if (isset($all['image']))
      {
      LanguageContent::InsertorUpdate($all['image'], PAGE_LANGUAGE, IMAGE_LANGUAGE, $id, 1, "sayfa");
}
   	return redirect()->back()->with('status', 'Bilgi Güncellendi');
	}
	else
	{
		return abort(404);
...
*/public function delete($id)
{
	...
	return redirect()->back()->with('status', 'Veri Başarı ile Silindi');
	}
	else
	{
		return abort (404);
...
LanguageContent::InsertorUpdate($all['slug'], PAGE_LANGUAGE, SLUG_LANGUAGE, $create->id, 0);
	...
	LanguageContent::InsertorUpdate($all['name'], PAGE_LANGUAGE, NAME_LANGUAGE, $create->id, 0);
   	...
   	if (isset($all['image']))
   	{
   	LanguageContent::InsertorUpdate($all['image'], PAGE_LANGUAGE, IMAGE_LANGUAGE, $create->id, 1 ,"sayfa");
}
   	return redirect()->back()->with('status', 'Veri Eklendi');
   		}
   		else
   		{
   			return redirect()->back()->with('status', 'Veri Eklenemedi');
   		}  
   }
